...
describe("Renvix Intelligence chat UI", () => {
  ...
  it("paces streamed copy and follows only the inner message viewport", async () => {
    ...
    expect(source).toContain("function observeAIStreamGrowth");
    expect(source).toContain("new ResizeObserver(() => queueAIMessageScroll())");
    expect(source).toContain("observeAIStreamGrowth(streamNode)");
    expect(source).toContain("stopObservingAIStreamGrowth(streamNode)");
    expect(source).toContain("event.deltaY < 0");
    expect(source).toContain("if (state.aiStreaming && movedUp) stopAIMessageFollowing()");
    expect(source).toContain("current.scrollTop = Math.max(0, current.scrollHeight - current.clientHeight)");
    expect(source).not.toContain('["wheel", "touchstart", "pointerdown"]');
  ...
  it("keeps AI data updates local and isolates ticket live refreshes from the chat", async () => {
    ...
    expect(source).toContain("renderOnComplete: !batchesInitialRender && !isAIPage && !refreshesDashboardProfile");
    ...
    expect(source).toContain('const refreshesDashboardProfile = target === "dashboardSessionProfile"');
    expect(source).toContain("if (state.route === routeAtStart) refreshDashboardProfileChrome()");
