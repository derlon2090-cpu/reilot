...
export async function adminListTickets(input = {}) {
  ...
  if (search) { values.push(`%${search}%`); where.push(`(t.subject ILIKE $${values.length} OR t.ticket_number ILIKE $${values.length} OR u.email ILIKE $${values.length} OR u.name ILIKE $${values.length})`); }
  values.push(limit, (page - 1) * limit);
  const rows = await query(`${selectTicket},u.name AS "requesterName",u.email AS "requesterEmail",tn.name AS "tenantName",
    auu.name AS "assignedAdminName",
    (SELECT body FROM support_ticket_messages m WHERE m.ticket_id=t.id AND NOT m.is_internal_note ORDER BY m.created_at DESC LIMIT 1) AS "lastMessage"
    FROM support_tickets t JOIN users u ON u.id=t.created_by_user_id JOIN tenants tn ON tn.id=t.tenant_id
...
export async function getAdminTicket(ticketId) {
  const ticket = await query(`${selectTicket},u.name AS "requesterName",tn.name AS "tenantName",auu.name AS "assignedAdminName"
    FROM support_tickets t JOIN users u ON u.id=t.created_by_user_id JOIN tenants tn ON tn.id=t.tenant_id
    LEFT JOIN admin_users au ON au.id=t.assigned_admin_user_id LEFT JOIN users auu ON auu.id=au.user_id
    WHERE t.id=$1 LIMIT 1`, [ticketId]);
  if (!ticket.rows[0]) return null;
  const messages = await query(`SELECT m.id,m.sender_type AS "senderType",m.body,m.is_internal_note AS "isInternalNote",m.created_at AS "createdAt",
...
export async function adminReply(admin, ticketId, input) {
  ...
  const result = await transaction(async (client) => {
    ...
    const ticket = locked.rows[0];
    if (!ticket) throw Object.assign(new Error("التذكرة غير موجودة."), { status: 404 });
    const message = await client.query(`INSERT INTO support_ticket_messages(ticket_id,tenant_id,sender_type,sender_admin_user_id,body,is_internal_note)
      VALUES($1,$2,'ADMIN',$3,$4,$5) RETURNING id`, [ticketId,ticket.tenant_id,admin.adminId,body,internal]);
  ...
  if (!internal) await createInAppNotification({ tenantId: result.ticket.tenant_id, userId: result.ticket.created_by_user_id, type:"support_reply", title:"تم الرد على رسالتك", message: body.slice(0,180), entityType:"support_ticket", entityId:ticketId, actionUrl:`/dashboard/support?ticket=${ticketId}`, dedupeKey:`support-reply:${result.id}` });
  return result;
}
