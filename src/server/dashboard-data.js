...
async function loadBaseStats(tenantId) {
  const result = await query(
    `SELECT
       ...
       (SELECT count(*) FROM whatsapp_channels WHERE tenant_id = $1 AND provider IN ('meta','meta_cloud','meta_cloud_api') AND status = 'connected')::int AS "connectedDevices",
       (SELECT count(*) FROM whatsapp_channels WHERE tenant_id = $1 AND provider IN ('meta','meta_cloud','meta_cloud_api') AND status IN ('connecting','pending_setup'))::int AS "pendingDevices",
       (SELECT max(last_health_check_at) FROM whatsapp_channels WHERE tenant_id = $1 AND provider IN ('meta','meta_cloud','meta_cloud_api')) AS "lastDeviceCheck",
