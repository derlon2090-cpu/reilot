...
Status: application fixes uploaded to draft PR #58 on branch `codex/security-hardening-20260919`; production app deployment, database migrations and host firewall installation have NOT been executed. On 2026-09-19 Cloudflare account IP blocks, zone WAF probe rule, API DNS proxy and Full (strict) TLS were applied and verified. Render/Vercel deployment credentials were not available in this workspace.
...
Additional live actions on 2026-09-19: Account IP Access Rules now block `195.178.110.72` and `45.115.26.203`. Zone WAF rule `renvix_probe_paths` blocks sensitive probe paths, excluding the isolated honeypot. `api.renvix.app` DNS is proxied and zone TLS is Full (strict). After each change, the public site and auth readiness returned 200, anonymous session 401, administration host still redirected to Access, honeypot stayed 200, and probe paths returned edge 403. The default `reilot.onrender.com` hostname still returned 200 for `/api/auth/readiness` and must be disabled in Render to close the bypass; application middleware denies it only after the PR is deployed. No firewall changes were made to the older Linux staging server because it is not the managed production API origin.
