...
test("explicit move button selects a container, supports touch targets and persists the new location", async ({ page }) => {
  ...
  ` });
  const move = page.getByRole("button", { name: "تحريك حسابات أبل الهند" });
