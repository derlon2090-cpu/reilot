...
describe("campaign creation controls", () => {
  ...
  it("accepts the supported email design", () => expect(campaignCreateSchema.safeParse(valid({ channel: "email", whatsappChannelId: null, subject: "عرض خاص", audienceFilter: { emailDesign: "showcase" } })).success).toBe(true));
  ...
  it("accepts independent email subject and body alignment", () => expect(campaignCreateSchema.safeParse(valid({ channel: "email", whatsappChannelId: null, subject: "عرض خاص", audienceFilter: { subjectAlignment: "center", bodyAlignment: "left" } })).success).toBe(true));
  it("rejects an unsupported email alignment", () => expect(campaignCreateSchema.safeParse(valid({ channel: "email", whatsappChannelId: null, subject: "عرض خاص", audienceFilter: { subjectAlignment: "justify" } })).success).toBe(false));
